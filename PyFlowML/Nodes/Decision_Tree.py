...
class Decision_Tree(NodeBase):
    def __init__(self, name):
        ...
        super(Decision_Tree, self).__init__(name)
    ...
    def compute(self, *args, **kwargs):
        ...
        # Convert y_test and y_pred to single-label format if they are in multilabel-indicator format
        if len(y_test.shape) > 1:
        ...
        metrics_table = pd.DataFrame(
            {'Accuracy (%)': [accuracy2], 'Precision (%)': [precision2], 'Recall (%)': [recall2], 'F1 Score (%)': [f12]})
        ...
        plt.savefig('metrics_tableDT.png')
        plt.close()

        self.metrics_table.setData('metrics_tableDT.png')
        ...
        plt.savefig('confusion_tableDT.png')
        plt.close()

        self.confusion_table.setData('confusion_tableDT.png')
